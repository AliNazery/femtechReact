...
import { AcheivementOneSvg, AcheivementTwoSvg } from "../Common/svg"; // Adjust paths based on your project structure
...
export default function Acheivement() {
  ...
  useEffect(() => {
    if (inView1 && countUpRefs[0].current) {
      new CountUp(countUpRefs[0].current, 90).start();
    }
    if (inView2 && countUpRefs[1].current) {
      new CountUp(countUpRefs[1].current, 50).start();
    }
    if (inView3 && countUpRefs[2].current) {
      new CountUp(countUpRefs[2].current, 30).start();
    }
    ...
    <section className="section relative z-20 overflow-hidden py-[60px] md:py-[100px]">
      <div className="container sm:px-6">
        <SectionMiddleTitle
          ...
          description="At FemTech, we're proud of the milestones we've reached, empowering women globally through technology and innovation."
        />
